...
public class RestRouteBuilder extends SpringRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        restConfiguration().component("jetty").host("127.0.0.1").port(8081).bindingMode(RestBindingMode.auto);
       ...
       //将http://127.0.0.1:8001/apis/users 接口 发布到 http://127.0.0.1:8081/camel/users
        ...
        rest("/users")
                ...
                .get().to("bean:userService?method=getAll")

                .get("/{id}")
                .param().name("id").type(RestParamType.path).endParam()
                .to("bean:userService?method=getById(${header.id})")
